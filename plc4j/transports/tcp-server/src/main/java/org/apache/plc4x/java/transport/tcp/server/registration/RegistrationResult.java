 ...
 * <h3>Usage Example:</h3>
 * <pre>{@code
 * RegistrationResult result = parser.parse(buffer);
 * switch (result.getStatus()) {
 *     case COMPLETE:
 *         String deviceId = result.getDeviceId();
 *         int consumed = result.getConsumedBytes();
 *         // Register device and continue processing
 *         break;
 *     case NEED_MORE_DATA:
 *         // Wait for more data to arrive
 *         break;
 *     case INVALID:
 *         String reason = result.getFailureReason().orElse("Unknown error");
 *         // Close connection
 *         break;
 * }
 * }</pre>
...
public final class RegistrationResult {
    ...
    public enum Status {
         ...
         * The buffer's reader index has been advanced by {@link #getConsumedBytes()} bytes.
         */
        COMPLETE,

        /**
         * Parser requires more data to complete.
         * The buffer's reader index remains unchanged.
         */
        NEED_MORE_DATA,
     ...
     *
     * <p>This method returns a cached instance for efficiency.</p>
     *
