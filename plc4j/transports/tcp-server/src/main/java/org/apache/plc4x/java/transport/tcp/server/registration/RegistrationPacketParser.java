 ...
 *
 * <p>Implementations must handle partial data scenarios gracefully, as Netty may
 * deliver registration frames in multiple pieces due to TCP segmentation.</p>
 *
 * <h3>Implementation Guidelines:</h3>
 * <ul>
 *   <li>Do not modify the buffer's reader index unless parsing succeeds</li>
...
public interface RegistrationPacketParser {
     ...
     *
     * <p>The parser should examine the readable bytes in the buffer and attempt
     * to extract a device identifier according to its configured format.</p>
     *
     * <p><strong>Buffer Management:</strong> Implementations must mark the reader
     * index before reading and reset it if parsing cannot complete. Only advance
     * the reader index when a complete registration packet is successfully parsed.</p>
     *
     * @param buffer the inbound Netty buffer positioned at the first byte of the
     *               expected registration packet; must not be null
     ...
     */
    RegistrationResult parse(ByteBuf buffer);
