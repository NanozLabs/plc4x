...
class Dlt645MessageCodecTest {

    private static final byte[] ADDRESS = {0x12, (byte) 0x90, 0x78, 0x56, 0x34, 0x12};

    @Test
    void skipsWakePreambleAndDispatchesFrame() throws Exception {
        var transport = new ScriptedTransport();
        var received = new ArrayList<Dlt645Frame>();
        var codec = new Dlt645MessageCodec(transport, received::add);
    ...
    void corruptFrameIsSkippedAndNextFrameParses() throws Exception {
        var transport = new ScriptedTransport();
        List<Dlt645Frame> received = new ArrayList<>();
        var codec = new Dlt645MessageCodec(transport, received::add);
    ...
    void partialAlignedFrameIsNotConsumed() throws Exception {
        ...
        transport.deliver(Arrays.copyOf(frame, 10));

        codec.processIncomingData();

        assertTrue(received.isEmpty());
        assertEquals(10, transport.getNumBytesAvailable());
    }
    ...
    void impossibleLengthDoesNotBlockAFollowingValidFrame() throws Exception {
        ...
        List<Dlt645Frame> received = new ArrayList<>();
        var codec = new Dlt645MessageCodec(transport, received::add);
        byte[] corruptHeader = Arrays.copyOf(wireFrame(), 10);
        corruptHeader[9] = (byte) 0xFF;
    ...
    void discardBufferedInputDropsLeftoverBytes() throws Exception {
        var transport = new ScriptedTransport();
        var codec = new Dlt645MessageCodec(transport, ignored -> { });
    ...
    private static Dlt645Frame frame() {
        return new Dlt645Frame(ADDRESS, ControlCode.READ_DATA_RESPONSE, (short) 8,
            new byte[]{0x00, 0x00, 0x01, 0x00, 0x56, 0x34, 0x12, 0x00});
    }

    private static byte[] wireFrame() throws Exception {
        Dlt645Frame frame = frame();
