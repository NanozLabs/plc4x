...
public class StaticHelper {
    ...
    private static final int HEADER_SIZE_WITHOUT_DATA = 11; // 68 + addr(7) + 68 + control + length
    private static final int FIXED_FRAME_OVERHEAD = 13;     // start + addr + start + control + length + cs + end
    ...
    /**
     * Checksum per CJ/T 188-2004 Section 6:
     * CS = mod-256 sum of all bytes from first 0x68 to before CS.
     * Includes: start1(0x68) + address(7) + start2(0x68) + control + length + wire data.
    ...
    /**
     * Find aligned frame start: first 0x68 whose +8 offset is also 0x68.
     */
    ...
    public static int findFrameStart(byte[] buffer, int fromOffset) {
        ...
        for (int i = start; i < buffer.length; i++) {
            ...
            if (i + SECOND_START_OFFSET >= buffer.length) {
                return -1;
            }
            if ((buffer[i + SECOND_START_OFFSET] & 0xFF) == FRAME_START) {
                return i;
            }
        }
        return -1;
    ...
    public static int estimateFrameLength(byte[] buffer, int frameStartOffset) {
        ...
        if (((buffer[frameStartOffset] & 0xFF) != FRAME_START) ||
            ((buffer[frameStartOffset + SECOND_START_OFFSET] & 0xFF) != FRAME_START)) {
            return -1;
        }
    ...
    /**
     * CJ/T 188-2004 error code bit definitions.
     ...
     * CJ/T 188 abnormal responses carry one ERR byte, optionally preceded by SER.
     */
    public static String describeError(byte[] dataPlain) {
        if (dataPlain == null || dataPlain.length == 0) {
            return "Error (invalid ERR data length=0)";
        }
        byte errByte = dataPlain.length == 1 ? dataPlain[0] : dataPlain[dataPlain.length - 1];
